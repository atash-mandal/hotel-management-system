...
class Booking:
    def __init__(self,root):
        ...
        self.bill_directory = self.current_working_directory+"\\bills"
        ...
        btn_bill=Button(btn_frame,text="Bill",command=self.total,font=("arial",12,"bold"),bg="black",fg="gold",width=9,height=1,cursor="hand2")
        btn_bill.grid(row=0,column=1,padx=1,pady=5)

        btn_down=Button(btn_frame,text="Download",command=self.download_bill,font=("arial",12,"bold"),bg="black",fg="gold",width=9,height=1,cursor="hand2")
        btn_down.grid(row=0,column=2,padx=1,pady=5)

        btn_add=Button(btn_frame,text="Add",command=self.add_data,font=("arial",12,"bold"),bg="black",fg="gold",width=9,height=1,cursor="hand2")
        btn_add.grid(row=1,column=0,padx=1,pady=10)

        btn_update=Button(btn_frame,text="Update",command=self.update_data,font=("arial",12,"bold"),bg="black",fg="gold",width=9,height=1,cursor="hand2")
        btn_update.grid(row=1,column=1,padx=1,pady=10)

        btn_delete=Button(btn_frame,text="Delete",command=self.delete_data,font=("arial",12,"bold"),bg="black",fg="gold",width=9,height=1,cursor="hand2")
        btn_delete.grid(row=1,column=2,padx=1,pady=10)

        btn_reset=Button(btn_frame,text="Reset",command=self.reset_data,font=("arial",12,"bold"),bg="black",fg="gold",width=9,height=1,cursor="hand2")
        ...
        btn_search=Button(tableframe,text="Search",command=self.search,font=("arial",10,"bold"),bg="black",fg="gold",width=21,cursor="hand2")
        btn_search.grid(row=0,column=3,padx=2)

        btn_show=Button(tableframe,text="Show All",command=self.fetch_data,font=("arial",10,"bold"),bg="black",fg="gold",width=20,cursor="hand2")
    ...
    def download_bill(self):
        if self.var_tax.get()=="":
            return
        my_path=self.bill_directory+'\\'+self.var_contact.get()+"_"+self.var_av_room.get()+".pdf"
        c = canvas.Canvas(my_path,bottomup=0,pagesize=letter)

        c.setFont("Courier-Bold",12)
        c.drawString(100,100,"Name")
        c.drawString(350,100,"XXXXXXX XXXXXX")

        c.drawString(100,125,"Email")
        c.drawString(350,125,"XXXXXXX XXXXXX")

        c.drawString(100,150,"Contact")
        c.drawString(350,150,self.var_contact.get())

        c.drawString(100,175,"Check-in")
        c.drawString(350,175,self.var_checkin.get())

        c.drawString(100,200,"check-out")
        c.drawString(350,200,self.var_checkout.get())

        c.drawString(100,225,"Meals")
        c.drawString(350,225,self.var_meals.get())

        c.setStrokeColorRGB(1,0,0) # red colour of line
        c.setLineWidth(2) #width of the line 
        c.line(100,250,450,250) # draw line 

        c.drawString(100,300,"Cost")
        c.drawString(350,300,self.var_subtotal.get())

        c.drawString(100,325,"Tax")
        c.drawString(350,325,self.var_tax.get())

        c.drawString(100,350,"Effective Cost")
        c.drawString(350,350,self.var_total.get())

        c.translate(inch,inch)
        c.drawImage("images\\logo.png",1*inch, 1*inch, width=1*inch, preserveAspectRatio=True)
        ...
        messagebox.showinfo("Success","Bill downloaded...")
